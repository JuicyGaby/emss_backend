...
exports.getUserAccessRightsById = async function (req, res, next) {
  try {
    const accessRightsId = req.params.id;
    const accessRights = await getUserAccessRightsById(accessRightsId);
    if (accessRights) {
      res.send(accessRights);
      return;
    }
    res.send({ error: "Access rights not found" });
  } catch (error) {
    console.error(error);
  }
};
